...
execute as @e[type=area_effect_cloud,tag=swPool_cueplace] at @s run function pool:classes/freeball/tp
...
execute if entity @e[tag=swPool_cue,tag=swPool_free] run scoreboard players set @e[tag=swPool_pooltable] swPool_time 0
...
execute if entity @e[tag=swPool_cue] run tag @a[tag=swPool_freeball] remove swPool_given
execute if entity @e[tag=swPool_cue] run tag @a[tag=swPool_freeball] remove swPool_freeball
