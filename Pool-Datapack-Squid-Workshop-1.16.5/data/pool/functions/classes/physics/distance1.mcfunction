#get HORIZONTAL distance from tag d1 to tag d2 with    1    as    upper limit     4 sig figs
#@s is tagged d1, only one d2 at a swPool_time
...
execute as @e[tag=swPool_d2] store result score D2 swPool_var01 run data get entity @s Pos[0] 10000
execute as @e[tag=swPool_d2] store result score D2 swPool_var02 run data get entity @s Pos[2] 10000
...
execute unless entity @e[tag=swPool_d2,distance=..1] run scoreboard players set @s swPool_dist 99999999
execute if entity @e[tag=swPool_d2,distance=..1] run scoreboard players operation @s swPool_dist = @s swPool_var00
